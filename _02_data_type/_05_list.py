...
before_id = id(lst)  # 이전 아이디

# list.append(값): list 끝에 값 추가
lst.append(999)
print("append 후 lst: ", lst)
print("append 후 lst id: ", id(lst))   # 기존 아이디는 동일한데 칸을 늘린것뿐
print("append 전후 같은 list 인가? ", before_id == id(lst))
...
# list update(수정)
# list[인덱스] = 값 -> 변수에 값 대입해서 변경하는 것과 똑같음
print("--- list update ---")
lst[0] = -10
print("lst: ", lst)
...
print(students[2])
print(len(students[0]))  # 2
print(len(students[2][0])) #4 (세종대왕 글자 수)
...
data = "홍길동, 20, 서울시, 서초구"
data_ = data.split(',')  # comma를 기준으로 인덱스를 나눔
...
for v in lst:
    print(v)   # v에서 앞에서부터 차례대로 본다. 즉, 하나하나 차례대로 옮겨간다.
...
# list.count(값): list 내에 <<같은 값>>이 몇 개 있는가?
print("--- list.count(값) ---")
...
fruits.sort(key=len)  # 정렬 기준이 len인거?
