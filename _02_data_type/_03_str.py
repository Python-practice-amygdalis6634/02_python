...
print('--- str.replace() ---')
today = '2026-06-01'
print(today, today.replace('-', '/'))  #old, new는 자동으로 나온다. 우리는 '-', '/' 이것만 입력하면 됨!
...
# 문자의 순서를 나타내는 번호 = index
# - sequence: 순차적인, 순서가 있는 데이터 구조
# - index : 순서 (base index == 0)

print("--- 문자열 indexing ---")
x = 'Monday'
print('x의 길이: ', len(x)) #길이 6, 인덱스 0~5
...
print("text[6:]: ", text[6:])  # 6 시작으로 끝까지
...
# id(변수명): 변수에 저장된 값의 주소(위치)를 변환
print("--- 문자열 불변 타입 ---")
s = 'python'   # s에는 'python' str 메모리 주소가 저장됨 (-696)
print("s: ", s)   # s에 저장된 주소를 찾아가서 'python' str을 참조
print("번경 전 s: ", id(s))

s = 'hello ' + s   # s가 저장된 위치가 달라진것 (-416)
print("s: ", s)
print("변경 후 s: ", id(s))
